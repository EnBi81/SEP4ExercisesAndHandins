import {ConfettiHelper, createOCanvas, createXCanvas, getCanvasUrl, Player, TicTacToeGame} from "./model";
...
export function TicTacToeGameObject(){
    return <TicTacToe game={game} player1={player1} player2={player2} canvasItems={canvasItems} confettiHelper={confettiHelper} />
}
...
function Sides({updateCount}){
    ...
    let playerNumber = game.isNoMovement() ? undefined : (game.isGameClosed() ? game.getWonPlayerNum() : game.getNextPlayerNumber());
    const isPlayer1 = playerNumber === player1.getPlayerNumber();
    const isPlayer2 = playerNumber === player2.getPlayerNumber();

    return (
        <div className="sides" data-update={updateCount}>
            <BackgroundSide classList={"side-left player1" + (isPlayer1 ? ' active' : '')}></BackgroundSide>
            <BackgroundSide classList={"side-right player2" + (isPlayer2 ? ' active' : '')}></BackgroundSide>
        </div>
    )
}
